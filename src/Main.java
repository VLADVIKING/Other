    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
                ...
                if (isOnSale[i]) {
                    sumProducts[i] = prices[i] * (counts[i] / 3 * 2 + counts[i] % 3);
                    sum += sumProducts[i];
                } else {
                    sumProducts[i] = prices[i] * counts[i];
                    sum += sumProducts[i];
                }
            }
        }
        ...
        for (int i = 0; i < products.length; i++) {
            System.out.println("\t" + products[i] + " " + (doBonus ? counts[i] + 1 : counts[i]) + " шт. за " + (sumProducts[i]) + " руб." + (isOnSale[i] ? " (распродажа!)" : ""));
        }
